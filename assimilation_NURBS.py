"""
statFEM_NURBS_batch.py — NURBS IGA data assimilation: batch rank test
=====================================================================
...
from surface_model import load_nurbs_from_npz
from surface_fem_solver import (
    assemble_nurbs_matrices, assemble_nurbs_load,
    _nurbs_eval, _geometry,
    l2_error, l2_error_nurbs, solve_poisson_zero_mean, solve_helmholtz,
)
...
# =====================================================================
# Helper: load NURBS and extract element spans                   
# =====================================================================
def load_nurbs(npz_path):
    """Load NURBS model and extract mesh info."""
...
# =====================================================================
# NURBS observation operator: interpolate at sensor param coords using NURBS basis
# =====================================================================
def build_observation_operator(nurbs, uv_sensors):
    """NURBS observation operator H: DOF space -> observation space."""
...
nurbs, elem_spans, n_ctrl_u, n_ctrl_v = load_nurbs(NURBS_NPZ)
n_dof = n_ctrl_u * n_ctrl_v
print(f"[IGA] DOFs={n_dof}, ctrl=({n_ctrl_u},{n_ctrl_v}), "
      f"deg=({nurbs.degree_u},{nurbs.degree_v})")

K, M = assemble_nurbs_matrices(nurbs, elem_spans, nq=NQ)
print(f"[IGA] K nnz={K.nnz}, M nnz={M.nnz}")

# --- Generate MMS data and sensor data ---
...
nurbs_for_sensor = load_nurbs_from_npz(NURBS_NPZ)
phys_coords, u_obs, u_true_sens, uv_sensors = simulate_sensors(
    nurbs_for_sensor, n_sensors=N_SENSORS, noise_std=NOISE_STD,
    k1=K1, k2=K2, rng=SEED,
)

# --- Interpolators and RHS vectors ---
u_vals = np.linspace(0.0, 1.0, N_GRID)
v_vals = np.linspace(0.0, 1.0, N_GRID)

f_noise_interp = RegularGridInterpolator((u_vals, v_vals), f_noisy, method='cubic')
f_clean_interp = RegularGridInterpolator((u_vals, v_vals), f_clean, method='cubic')
u_true_interp  = RegularGridInterpolator((u_vals, v_vals), u_true_grid, method='cubic')

F_noisy_vec = assemble_nurbs_load(nurbs, elem_spans, f_noise_interp, nq=NQ)
F_clean_vec = assemble_nurbs_load(nurbs, elem_spans, f_clean_interp, nq=NQ)

# L2 projection of u_true into NURBS space
F_true_nodes = assemble_nurbs_load(nurbs, elem_spans, u_true_interp, nq=NQ)
u_true_nodes = spsolve(M.tocsc(), F_true_nodes)
...
L2_err_base, L2_true = l2_error_nurbs(
    nurbs, elem_spans, u_base, u_true_interp, nq=NQ)
...
def run_assimilation(K, M, F_noisy_vec, Vr, H, y_obs,
    ...
    """
    Core assimilation solve: full form
    """
    n_dof = K.shape[0]
    N_obs = H.shape[0]
    ...
    u_post = sol[:n_dof]
    alpha  = sol[n_dof:]
    Cov_alpha = np.linalg.inv(Lambda_22)
    return u_post, alpha, Cov_alpha
...
for r_val in R_VALUES:
    ...
    u_post_analytical, _, _ = run_assimilation(
    ...
    # NURBS uses integral for exact error
    L2_err_analytical, _ = l2_error_nurbs(
        nurbs, elem_spans, u_post_analytical, u_true_interp, nq=NQ)
    ...
    # 5. Run best Optuna assimilation (integral error)
    Sigma_alpha_inv_best = np.eye(r_val) * best['alpha_pen']
    u_post_optuna, alpha_optuna, Cov_alpha_optuna = run_assimilation(
        K, M, F_noisy_vec, Vr, H, u_obs,
    ...
    L2_err_optuna, _ = l2_error_nurbs(
        nurbs, elem_spans, u_post_optuna, u_true_interp, nq=NQ)
